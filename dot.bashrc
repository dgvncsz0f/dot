...
my_ssh_add()
{
  if [ -n "$SSH_AGENT_PID" ]
  then
    $HOME/.dot/bin/cache-pw /bin/sh -c 'for f in $(/usr/bin/find "$HOME/.ssh/" -xtype f); do if /usr/bin/file -b "$f" | /bin/grep -q -i "\bprivate key\b"; then if ! /usr/bin/ssh-add -l | /bin/grep -q "$f"; then /usr/bin/ssh-add "$f"; fi; fi; done'
  fi
}
